...
public class OnCourseAdsClientsPanel extends javax.swing.JPanel {
    ...
    public void contractChanged() {
        if (isClientSelected()) {
            /* 
                old code commented by javed on 05-04-19
                for immediate update UI bug
             */
...
//            }
            ...
            /* added by javed on 05-04-19 
               for updating the UI of Client active/inactive
             */
            if (selectedClient != null && selectedClient.getAdses().size() == 0) {
                selectedClient.setActive(false);
                
            } else if (selectedClient != null && selectedClient.getAdses().size() > 0) {
                boolean haveContract = false;
                for (Ads tempAd : selectedClient.getAdses()) {
                    if (tempAd.getContractses().size() == 0) {
                        haveContract = false;
                    } else {
                        haveContract = true;
                        break;
                 ...
                 selectedClient.setActive(haveContract);
                boolean activeClient = false;
                for (Ads ad : selectedClient.getAdses()) {
                    
                    if (ad.isActive()) {
                        //selectedClient.setActive(Boolean.FALSE);
                        activeClient = true;
                        break;
                    }
                    ...
                    selectedClient.setActive(activeClient);
    ...
    /**
     * Set the selected client fields from the table and components. Avoid
     * multiple calling of this method from the table changed events.
     */
...
//    @Action(enabledProperty="clientSelected")
    public Task deleteClient() {
        ...
        if (internet == 1) {
            ...
            if (answer == JOptionPane.YES_OPTION) //	    storageTasks.deleteClient(selectedClient);
            {
    ...
    private static class ClientsTableFormat implements AdvancedTableFormat<Clients>, WritableTableFormat<Clients> {
        ...
        public Class getColumnClass(int column) {
            int i = 0;
            if (column == i++) {
                return String.class; //cartNumber
            }
            if (column == i++) {
                return Boolean.class; //active
            }
            if (column == i++) {
                return String.class; //ads
            }
            if (column == i++) {
                return Date.class; //client since
            }
        ...
        public Object getColumnValue(Clients client, int column) {
            ...
            if (column == i++) {
                return client.getActiveAdsCount() + "/" + client.getAdsCount();
            }
            if (column == i++) {
        ...
        public Clients setColumnValue(Clients client, Object editedValue, int column) {
            if (column == 0) {
                client.setName((String) editedValue);
            }
            return client;
        }
    }
