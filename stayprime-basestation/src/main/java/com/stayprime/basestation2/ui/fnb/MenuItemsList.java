...
public class MenuItemsList extends ListEditor<MenuItems> {
    ...
    public MenuItemsList() {
        ...
        //  List<MenuItems>me= getSourceList();
        setItemFactory(new Factory<MenuItems>() {
            @Override
            public MenuItems create() {
                if (getSourceList() != null && getSourceList().size() > 0) {
                    for(MenuItems item:getSourceList()){
                    String[] name = {};
                    if (item != null) {
                        if (item.getDescription() != null) {
                            name = item.getDescription().split(";");
                        }

                        if (item.getName() == null || StringUtils.isBlank(item.getName()) || item.getDescription() == null || StringUtils.isBlank(name[0])) {
                            NotificationPopup.showIncompeleteDetailsErrorPopup("Menu Items Detail Not Complete");
                            return null;
                        }
                    }
                }
