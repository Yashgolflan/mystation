...
public class CartTrackingPanel extends JPanel {
    ...
    private boolean popupShown=Boolean.FALSE;
    ...
    private void addChangeListeners() {
        ...
        ActionListener actionListener = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                //    nextDateButton.setEnabled(Boolean.TRUE);
                Date now = new Date();
                now.setHours(0);
                now.setMinutes(0);
                now.setSeconds(0);
                if (datePicker.getDate().toString().equals(now.toString())) {

                    nextDateButton.setEnabled(Boolean.FALSE);
                } else {
                    nextDateButton.setEnabled(Boolean.TRUE);
                }
                updateTrack();
...
//        }
        ...
        //  jToggleButton1.setAction(Application.getInstance(BaseStation2App.class).getContext().getActionMap(MainPanel.class, this).get("showDashboardPanel"));
        clearTrack();
    }
...
//	if(datePicker.getDate() == null) {
        ...
        datePicker.getMonthView().setUpperBound(now);
        nextDateButton.setEnabled(Boolean.FALSE);
//	}
    }
    ...
    private class ShowTrackTask extends org.jdesktop.application.Task<List<CartTracking>, Void> {
        ...
        ShowTrackTask(org.jdesktop.application.Application app) {
            ...
            // jToggleButton1.setAction(Application.getInstance(BaseStation2App.class).getContext().getActionMap(MainPanel.class, CartTrackingPanel.this).get("showDashboardPanel"));
        }

        @Override
        protected List<CartTracking> doInBackground() {
            setMessage("Loading...");
        ...
        @Override
        protected void succeeded(List<CartTracking> track) {
          if(popupShown==false){  
            if (connectTrack) {
                trackRenderer.setCartTrack(cart, track);
            } else {
                trackRenderer.setPoints(track);
            }
            ...
            if(track.size()==0||track==null){
                //int result= JOptionPane.s
                popupShown=Boolean.TRUE;
                NotificationPopup.showIncompeleteDetailsErrorPopup("Not Data Found");
            }
