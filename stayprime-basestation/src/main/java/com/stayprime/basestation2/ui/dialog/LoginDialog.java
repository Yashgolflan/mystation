...
public class LoginDialog extends AbstractConnectionProvider {
    ...
    private ActionListener actionListener = new AbstractAction() {
        ...
        public void actionPerformed(ActionEvent e) {
            int result=checkNetworkConnectivity();
            if(result==1)
                login();
        }
    };
    ...
    private void login() {
         ...
         * Do something
                     */
                    user = courseService.authUser(getUsername(), getPassword());
                    if (btncancelFlag == true) {
                        btncancelFlag = false;
                        return;
                    }
                    if (user != null) {
...
//            }
            ...
            NotificationPopup.showErrorPopup("No Internet Connection\n Please check the the Internet Connection");
            System.out.println("Login Dialog" + e.toString());
        }
    }
