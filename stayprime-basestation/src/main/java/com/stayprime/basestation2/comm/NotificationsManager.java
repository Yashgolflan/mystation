...
//            throw new RuntimeException("Error sending reply " + ex, ex);
   ...
   //     List<ServiceRequest> requests = cartService.listServiceRequests(RequestType.REQUEST_SERVICED);
      ...
      List<ServiceRequest> requests = cartService.listServiceRequests(RequestType.REQUEST_SERVICED); 
        ...
        //log.info("javed 22 " + requests.size());
            ...
            if (request.getType() == RequestType.FNB.getType()) {
                ...
                for (FnbOrder fnb : fnbOrders) {
                    ...
                    if (fnb.getCartNumber() == request.getCartNumber()
                            &&
                            fnb.getSendStatus().getCreated()!=null
                            &&
                            fnb.getSendStatus().getCreated().equals(request.getTime())) {
//                        additionalInfo = fnb.getItems();          /* old code commented by javed on 09-04-19 for notification message code content*/
                        if (fnb.getSendContent() == null) { /* whole if added by javed on 09-04-19 for notification detailed message content*/
                            additionalInfo = fnb.getItems();
                        } else {
                            additionalInfo = fnb.getSendContent();
                        }
        ...
        //log.info("javed 33 request loop complete");
    }
