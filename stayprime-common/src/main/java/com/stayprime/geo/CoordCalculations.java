...
import com.stayprime.util.geometry.Rect;
 ...
 * @author benjamin
 */
public class CoordCalculations {

    public static final double R = 6378137.0;

    public static Xy getOrthographicProjectionInMeters(Coordinates coord, Coordinates centerPoint) {
    ...
    public static Xy getMercatorProjectionInMeters(Coordinates coord, Coordinates falseCenter) {
        return getMercatorProjectionInMeters(coord.latitude, coord.longitude, falseCenter);
    }

    public static Xy getMercatorProjectionInMeters(double latitude, double longitude, Coordinates falseCenter) {
        ...
        Xy pos = new Xy((x - x0)*scale0, (y - y0)*scale0);
        return pos;
    }


    public static Xy getXYRelativeToMapPixelCoordinates(
	    Coordinates coord, BasicMapImage map, Rect rect) {
	return getXY(coord, map, new Rect(0, 0, rect.getWidth(), rect.getHeight()));
    }

    public static Xy getXY(Coordinates coord, BasicMapImage map, Rect rect) {
        return getXY(coord.latitude, coord.longitude, map, rect);
    }

    public static Xy getXY(double lat, double lon, BasicMapImage map, Rect rect) {
	...
	Xy topLeft = getMercatorProjectionInMeters(map.getTopLeft(), center);
	Xy topRight = getMercatorProjectionInMeters(map.getTopRight(), center);
        Xy bottomLeft = getMercatorProjectionInMeters(map.getBottomLeft(), center);
        Xy bottomRight = getMercatorProjectionInMeters(map.getBottomRight(), center);
        Xy point = getMercatorProjectionInMeters(lat, lon, center);
	
	point.set(point.getX() - topLeft.getX(), point.getY() - topLeft.getY());
	topRight.set(topRight.getX() - topLeft.getX(), topRight.getY() - topLeft.getY());
	bottomLeft.set(bottomLeft.getX() - topLeft.getX(), bottomLeft.getY() - topLeft.getY());
	bottomRight.set(bottomRight.getX() - topLeft.getX(), bottomRight.getY() - topLeft.getY());
	topLeft.set(0, 0);
    ...
    public static Coordinates getCoordinatesFromXYRelativeToMapPixel(
            Xy pointPix, BasicMapImage map, Rect rect) {
	...
	Xy topLeft = getMercatorProjectionInMeters(topLeftCoord, center);
	Xy topRight = getMercatorProjectionInMeters(map.getTopRight(), center);
        Xy bottomLeft = getMercatorProjectionInMeters(map.getBottomLeft(), center);
	...
	Xy point = new Xy(topLeft.getX() + x, topLeft.getY() + y);
    ...
    public static float getTopEdgeAngle(
        ...
        //Xy bottomRight = getOrthographicProjectionInMeters(bottomRightCoordinate, topLeftCoordinate);
        Xy bottomRight = getMercatorProjectionInMeters(bottomRightCoordinate, center),
    ...
    public static double getAngleRad(Coordinates origin, Coordinates point) {
        Xy bottomRight = getMercatorProjectionInMeters(point, origin);
    ...
    public static double distanceInMeters(double lat1, double lon1, double lat2, double lon2) {
        ...
        return (d);
        /*Xy p1 = getMercatorProjectionInMeters(coord1, coord1),
    ...
    public static double distanceInMeters(Coordinates coord1, Coordinates coord2, Coordinates center) {
        Xy p1 = getMercatorProjectionInMeters(coord1, center),
    ...
    public static Xy fastProjection(Coordinates origin, Coordinates coord) {
	double latCorrection = getLatitudeCorrection(origin.latitude);
	return new Xy(Math.toRadians(coord.longitude - origin.longitude) * R,
    ...
    /*private static Coordinates getInverseOrtographicProjectionFromMeters(Xy point, Coordinates centerPoint) {
        double lat0 = Math.PI / 180 * centerPoint.latitude,
                lon0 = Math.PI / 180 * centerPoint.longitude,
    ...
    public static Coordinates getInverseMercatorProjectionFromMeters(Xy point, Coordinates falseCenter) {
        double toRad = Math.PI / 180, toDeg = 180 / Math.PI;
